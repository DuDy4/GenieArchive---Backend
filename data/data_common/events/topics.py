class Topic:
    ...
    NEW_EMAIL_ADDRESS_TO_PROCESS = (
        "new-email-address-to-process"  # Derived from new meetings
    )
    NEW_LINKEDIN_URL = "new-linkedin-url"  # Not implemented yet - should notify about manually added LinkedIn urls

    # Events that require tasks

    # People enrichment
    NEW_EMAIL_ADDRESS_TO_ENRICH = (
        "new-email-address-to-enrich"  # Call for people data labs
    )
    NEW_CONTACT_TO_ENRICH = "new-contact-to-enrich"  # Call for people data labs
    UPDATED_ENRICHED_DATA = (
        "got-updated-enriched-data"  # People data labs succeeded in enriching data
    )
    NEW_PERSONAL_DATA = "new-personal-data-to-process"  # PersonManager calls for Langsmith to process enriched data
    NEW_PROCESSED_PROFILE = (
        "new-processed-profile"  # Langsmith Succeeded in processing enriched data
    )
    ...
    COMPANY_NEWS_UPDATED = (
        "company-news-updated"  # News scrapper succeeded in getting news
    )
    ...
    FAILED_TO_ENRICH_DATA = (
        "failed-to-enrich-data"  # People data labs failed to enrich data
    )
    FAILED_TO_ENRICH_EMAIL = (
        "failed-to-enrich-email"  # People data labs failed to enrich email
    )
    FAILED_TO_GET_DOMAIN_INFO = (
        "failed-to-get-domain-info"  # Hunter.io failed to get domain info
    )
    FAILED_TO_GET_LINKEDIN_URL = "failed-to-get-linkedin-url"  # NOT ACTIVE - Langsmith failed to get LinkedIn url
    FAILED_TO_GET_COMPANY_NEWS = (
        "failed-to-get-company-news"  # News scrapper failed to get news
    )
