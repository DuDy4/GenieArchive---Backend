...
from data.data_common.data_transfer_objects.profile_dto import NewsData, ProfileDTO, Connection, Hobby
...
if email:
    ...
    if person:
        ...
        existing_news = profiles_repository.get_news_data_by_email(email)
            ...
            if st.form_submit_button("Save"):
                ...
                if (existing_hobbies != hobbies_list):
                    ...
                    for hobby in hobbies_list:
                        hobby_uuid = hobbies_repository.find_or_create_hobby(hobby["hobby"], hobby["icon_url"])
                        hobby_from_db = hobbies_repository.get_hobby(hobby_uuid)
                        if not hobby_from_db["icon_url"] == hobby["icon_url"]:
                            hobbies_repository.update_icon_url(hobby_uuid, hobby["icon_url"])
                        hobby_uuids.append(hobby_uuid)
                    profiles_repository.update_hobbies_by_email(email, hobby_uuids)
                top_news_list = [{"title": n[0], "link": n[1], "media": n[2]} for n in news_inputs if n[0] and n[1] and n[2]]
                if (existing_news != top_news_list):
                    ...
                    profiles_repository.update_news_by_email(email, filtered_list) 
                else:
                    logger.info("News list is the same")
                relevant_connections_list = [{"name": c[0], "image_url": c[1], "linkedin_url": c[2]} for c in connection_inputs if c[0] and c[1] and c[2]]
                if (existing_connections != relevant_connections_list):
                    logger.info(f"New connections: {relevant_connections_list}")
                    profiles_repository.update_connections_by_email(email, relevant_connections_list)
                
                st.success("Prospect data updated successfully!")

        if st.button("Add Hobby"):
            st.hobby_inputs.append(Hobby(hobby_name="Sleeping", icon_url="https://icon.com"))
            st.experimental_rerun()

        if st.button("Add News"):
            st.news_inputs.append(NewsData(link="https://linkedin.com/my-post", title="My Post", media="linkedin"))
            st.experimental_rerun()

        if st.button("Add Connection"):
            st.connection_inputs.append(Connection("John Doe", "", "" ))
            st.experimental_rerun()
