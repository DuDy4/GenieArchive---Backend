...
if email:
    ...
    if person:
        ...
        if not hasattr(st, 'hobby_inputs'):
            st.hobby_inputs = existing_hobbies if existing_hobbies else [({"hobby_name" : "", "icon_url" : ""})]
        if not hasattr(st, 'news_inputs'):
            logger.info(f"Existing news: {NewsData.to_json(existing_news)}")
            st.news_inputs = [existing_news] if existing_news else [(NewsData("", "", "linkedin"))]
        if not hasattr(st, 'connection_inputs'):
            st.connection_inputs = existing_connections if existing_connections else [({"name" : "", "image_url" : "", "linkedin_url" : ""})]
        ...
        with st.form("enrichment_form"):
            ...
            if st.form_submit_button("Save"):
                ...
                if (existing_news != top_news_list):
                    ...
                    profiles_repository.update_news_by_email(email, top_news_list) 
                else:
